...
; 怎么定义? todo
; 暂时这么定义吧, 用procedure representation表示感觉不太好实现environment?谓词
...
(define (extend-env idents scheme_vals saved_env)
...
(define (extend-env-rec listof_proc_name listof_bound_vars listof_proc_body saved_env)
  ...
  (define (env search_var)
      (let ((search_result (search-proc-aux search_var 
                                            ...
                                            listof_bound_vars
                                            listof_proc_body
                                            env)))  ; 注意是外层的env, env在这里是闭包(宿主语言的), 会把必须的bound_vars, proc_body等信息在宿主语言保存起来
                                                    ; 应该类似于eopl实现proc, (define (env search_var)) 宿主语言scheme能把环境里的free variables保存起来(capture)
