...
(eopl:pretty-print (run
  "if <(11, 12) then 30 else 40"
))
(eopl:pretty-print (run  "if zero?(-(11, 11)) then 31 else 41"))
(eopl:pretty-print (run "minus(4)"))
(eopl:pretty-print (run  "if zero?(-(11, 11)) then minus(3) else minus(4)"))
(eopl:pretty-print (run "+(1, 2)"))
(eopl:pretty-print (run "+(+(1,2), 3)"))
(eopl:pretty-print (run "/(1, 2)"))
(eopl:pretty-print (run "*(*(1,2), *(10, 2))"))
(eopl:pretty-print (run "cond <(2, 1) ==> 1 >(2, 2) ==> 2  >(3, 2) ==> 3 end"))
...
; 目前if cond 都是以(not (= (expval->num val) 0))为准, 只有当val就是0的时候, 才会是false, 其他都是true
; 目前不能出现 if (list 1 2 3)这种, 必须是num-val构造出来的, 不能expval->num会报错