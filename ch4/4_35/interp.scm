  ...
  (define value-of
    (lambda (exp env)
      (cases expression exp
        ...
        (ref-exp (ident)
          (ref-val (apply-env env ident)))  ; 得到ref

        (deref-exp (ident)
          (deref (expval->ref (deref (apply-env env ident)))))
        
        (setref-exp (ident exp1)
          (let ((ref1 (apply-env env ident))
                (val1 (value-of exp1 env)))
            (setref! (expval->ref (deref ref1)) val1)))
