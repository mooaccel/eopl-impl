...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
  ...
  ; 返回refs
  (define (new-initialized-references-aux idents vals)
    (if (null? idents)
        '()
        (cons (newref (car vals))
              (new-initialized-references-aux (cdr idents) (cdr vals)))))
            ...
            (if (expval->bool val1)
                (begin 
                  ...
                  (result-of (while-stat exp1 stat1) 
                             env))
                'finish)))  ; 随便放个返回值都行
        (declare-stat (idents exps stat1)  ; 如果idents为空, todo
          (let ((vals (map (lambda (exp_item) 
                              (value-of exp_item env))
                           exps)))
            (let ((refs (new-initialized-references-aux idents vals)))
              (result-of stat1 
