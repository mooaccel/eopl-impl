    ...
    (lambda (exp env cont)  ; cont是exp的continuation
      (begin
        ...
        (cases expression exp
          ...
          (yield-exp ()
            (let ((cur_timeslice_remaining (get-the-time-remaining)))
              (place-on-ready-queue!
                (lambda ()
                  (set-the-time-remaining! cur_timeslice_remaining)
                  (eopl:printf "resume from yield-exp, cur remaning = ~s~%" (get-the-time-remaining))
                  (apply-cont cont
                              (num-val 99))))
              (run-next-thread)
            ))
  ...
  (define (apply-cont cont val) 
    ...
    (if (time-expired?)
        (begin
          ...
          (place-on-ready-queue!
            (lambda ()
              (eopl:printf "resume from normal time-expired, cur remaning = ~s~%" (get-the-time-remaining))
              (apply-cont cont val)))
          (run-next-thread))
        (begin
          (decrement-timer!)
