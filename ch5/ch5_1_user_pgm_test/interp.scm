      ...
      (begin
        ...
        (cases expression exp
          ...
          (multiplication-exp (exp1 exp2)
            (value-of/k exp1
                        env
                        (multiplication1-cont exp2 env cont))) ; 多久触发这个diff1-cont? 在exp1得到结果后, apply-cont
      ...
      (cases continuation cont 
        (end-cont () 
          ...
          (multiplication1-cont (exp2 env saved_cont)
            (value-of/k exp2
                        env
                        (multiplication2-cont val ; exp1的结果存起来
                                    saved_cont)))         
          (multiplication2-cont (val1 saved_cont)
            (eopl:printf "In apply-cont multiplication2-cont, val1 = ~s, val2 = ~s, saved_cont = ~s ~%" val1 val saved_cont)
            (let ((num1 (expval->num val1))
                  (num2 (expval->num val)))
              (apply-cont saved_cont
                          (num-val (* num1 num2)))))
