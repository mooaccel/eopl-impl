(module interp (lib "eopl.ss" "eopl")
  ...
  (provide value-of-program 
           ...
           instrument_value_of_k_call_exp
           instrument_apply_cont)

  (define instrument_value_of_k (make-parameter #f))
  (define instrument_value_of_k_call_exp (make-parameter #f))
...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
  ...
  (define (size-of-cont cont)
    (cases continuation cont 
        (end-cont () 
          1)
        (zero1-cont (saved_cont) 
          (+ 1 (size-of-cont saved_cont)))
        ...
        (list-exp-cont-remaining (car_val saved_cont)
          (+ 1 (size-of-cont saved_cont)))
        (let-exp-cont (vars body env saved_cont)
          (+ 1 (size-of-cont saved_cont)))
    ))
      ...
      (begin
        (if (instrument_value_of_k)
            (begin
              ...
              (eopl:printf "size of cont = ~s ~%" (size-of-cont cont)))
            'ignore)
        (cases expression exp
          ...
          (call-exp (rator rands)
            (begin
              (if (instrument_value_of_k_call_exp)
                  (begin
                    (eopl:printf "In value-of/k call-exp, rator = ~s rands = ~s ~%" rator rands)
                    (eopl:printf "In value-of/k call-exp size of cont = ~s ~%" (size-of-cont cont)))
                  'ignore)
              (value-of/k rator
                          env
          ...
          (letrec-exp (proc_name bound_vars proc_body letrec_body)
            (value-of/k letrec_body
                        (extend-env-rec proc_name bound_vars proc_body env)
                        cont))
    ...
    (begin
      ...
      (cases continuation cont 
        (end-cont () 
          ...
          (rator-cont (rands env saved_cont)
            (value-of/k (list-exp rands)
                        env
                        (rands-cont val saved_cont)))
          (rands-cont (rator saved_cont)
            (let ((proc1 (expval->proc rator)))
              (apply-procedure proc1 (transform-aux val) saved_cont)))
          (cons-exp-cont-1 (exp2 env saved_cont)
            (value-of/k exp2
                        env
