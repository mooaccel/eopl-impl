  ...
  (define value-of/k
    (lambda (exp env cont)  ; cont是exp的continuation
      (cases expression exp
        ...
        (try-exp (exp1 excep_var cont_var handler_exp)  ; cont是try-exp的continuation
          ;(eopl:pretty-print "In try-exp")
          ;(eopl:pretty-print cont)
          (value-of/k exp1  ; 正常值或exception.最后送入try-cont
                      env
                      (try-cont excep_var cont_var handler_exp env cont)))  ; 进入try-cont的方式有两种, apply-cont或者apply-handler
    ...
    (cases continuation cont 
        ...
        (rand-cont (rator saved_cont)
          (cases expval rator
            ...
            (cont-val (cont1)
              (apply-cont cont1 val))       ; 效果和之前的一样, 只不过选择了这条路继续cont
