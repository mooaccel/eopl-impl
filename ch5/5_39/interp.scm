...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
      ...
      (cases expression exp
        ...
        (try-exp (exp1 var handler_exp)  ; cont是try-exp的continuation
          ...
          (value-of/k exp1  ; 正常值或exception.最后送入try-cont
                      env
                      (try-cont var handler_exp cont)))  ; 进入try-cont的方式有两种, apply-cont或者apply-handler
  ...
  (define (apply-cont cont val) 
    (cases continuation cont 
        ...
        (try-cont (var handler_exp saved_cont)
          (apply-cont saved_cont val))  ; 通过apply-cont try-cont val到这里, 说明是正常值

        (raise-cont (saved_env saved_cont)
          (apply-handler val saved_cont saved_env))  ;处理val, 即exception

    ))

  (define apply-handler
    (lambda (val cont env)   ; 参数顺序无所谓吧
      (cases continuation cont 
        (try-cont (var handler_exp saved_cont)
